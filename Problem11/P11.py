...
# build matrix
string = "08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08 49 49 99 40 17 81 18 57 60 87 17 40 98 43 69 48 " \
         ...
         "89 19 67 48 "
stringArray = string.split()  # into 1-d matrix of strings
M = []
for i in range(len(stringArray)):  # into 1-d matrix of ints
    M.append(int(stringArray[i]))
matrix = np.array_split(M, 20)  # into 2-d array of ints

maxProduct = 0
currentProduct = 1

# vertical check
for i in range(0, 17):  # vertical start index
    for j in range(0, 20):  # horizontal index
...
matrix = np.transpose(matrix)
for i in range(0, 17):  # vertical start index
    for j in range(0, 17):  # horizontal index
...
# / check
for i in range(0, 17):  # vertical start index
    for j in range(3, 20):  # horizontal index
        currentProduct = matrix[i][j] * matrix[i + 1][j - 1] * matrix[i + 2][j - 2] * matrix[i + 3][j - 3]
        if currentProduct > maxProduct:
            maxProduct = currentProduct
print(maxProduct)
